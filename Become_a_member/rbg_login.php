...
if(isset($_SESSION['user_id'])){
	header("http://localhost/RBGym/Become_a_member/rbg_index_login.php");
}
...
if (!empty($_POST['email']) && !empty($_POST['password'])):
	...
	if(count($results) > 0 && $_POST['password'], strlen($results['password'])) == 0){
		// creates session id
		...
		// error message
		$output = 'Sorry those credentials do not match, please try again.'.'<br><a href="http://localhost/RBGym/member/rbg_login.php"><button class="w3-btn">Go back</button><a><br>';
		die($output);
		}
...
<?php endif; ?>
	...
	<h1>Login</h1>
	<span>or <a href="rbg_reg_mem.php">register here</a></span>

	<form action="rbg_login.php" method="POST" accept-charset="utf-8">
